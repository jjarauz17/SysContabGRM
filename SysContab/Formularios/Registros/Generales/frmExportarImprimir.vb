...
Public Class frmExportarImprimir
        ...
        If Guardar.ShowDialog = DialogResult.OK Then
            ...
            If Extension = "*.xlsx" Or Extension = "*.xls" Then
                ...
                If TypeOf (GridP) Is DevExpress.XtraGrid.GridControl Then _
                    CType(GridP, DevExpress.XtraGrid.GridControl).ExportToXlsx(Guardar.FileName, New XlsxExportOptionsEx With {.ExportType = ExportType.Default})

                'If Extension = "*.xlsx" Then CType(GridP, DevExpress.XtraGrid.GridControl).ExportToXlsx(Guardar.FileName, New XlsxExportOptionsEx With {.ExportType = ExportType.WYSIWYG})
                'If Extension = "*.xls" Then CType(GridP, DevExpress.XtraGrid.GridControl).ExportToXls(Guardar.FileName, New XlsExportOptions With {.TextExportMode = TextExportMode.Text})
                'If Extension = "*.xls" Then CType(GridP, DevExpress.XtraGrid.GridControl).ExportToXls(Guardar.FileName, New XlsExportOptions With {.ExportMode = XlsExportMode.SingleFile})
                ' If Extension = "*.xls" Then GridP.ExportToText(Guardar.FileName)
                ' End If
                ...
                If TypeOf (GridP) Is DevExpress.XtraPivotGrid.PivotGridControl Then
                    ...
                    Try
                        ...
                        ' If Extension = "*.xlsx" Then iPivot.ExportToXlsx(Guardar.FileName, options)
                        ' If Extension = "*.xls" Then iPivot.ExportToXls(Guardar.FileName)
                ...
                'Si es un Arbol
                If TypeOf (GridP) Is DevExpress.XtraTreeList.TreeList Then
                    If Extension = "*.xlsx" Then CType(GridP, DevExpress.XtraTreeList.TreeList).ExportToXlsx(Guardar.FileName, New XlsxExportOptionsEx With {.ExportType = ExportType.WYSIWYG})
                    If Extension = "*.xls" Then GridP.ExportToXls(Guardar.FileName)
                End If

                'Si es una Vista
                If TypeOf (GridP) Is DevExpress.XtraGrid.Views.Grid.GridView Then
                    If Extension = "*.xlsx" Then CType(GridP, DevExpress.XtraGrid.Views.Grid.GridView).ExportToXlsx(Guardar.FileName, New XlsxExportOptionsEx With {.ExportType = ExportType.WYSIWYG})
                    If Extension = "*.xls" Then GridP.ExportToXls(Guardar.FileName)
                End If
    ...
    Private Sub bPrevia_Click(sender As Object, e As EventArgs) Handles bPrevia.Click
        VistaPrevia(GridP, Titulo_Print, Periodo_Print, rgOrientacion.EditValue)
    End Sub
