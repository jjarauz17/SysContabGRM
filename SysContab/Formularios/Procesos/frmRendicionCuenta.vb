...
Public Class frmRendicionCuenta
    ...
    Sub CargarCombos()
        ...
        SearchLookUp(cbRendirCuenta, ObtieneDatos("JAR_GetCatalogoRendicion", EmpresaActual), "Display", "Cuenta", 2)
    ...
    Private Sub SimpleButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SimpleButton1.Click
        ...
        If txtReferencia.Text.Trim.Length = 0 Then
            XtraMsg("Digite la Referencia de esta transacción", MessageBoxIcon.Warning)
            txtReferencia.Focus()
            Exit Sub
        End If
        '
        If cbRendirCuenta.EditValue Is Nothing Then
            XtraMsg("Seleccione la Cuenta a Rendir!", MessageBoxIcon.Warning)
            cbRendirCuenta.Focus()
            Exit Sub
        End If

        If cbTipo.EditValue Is Nothing Then
            XtraMsg("Seleccione Tipo de Comprobante", MessageBoxIcon.Warning)
            cbTipo.Focus()
            Exit Sub
        End If
        ...
        Try
            Dim CompNo As Integer = VB.SysContab.ComprobanteDB.AddComprobante(cbTipo.EditValue, TCambio,
                                                                              txtComentario.Text, MonedaBase,
                                                                              VB.SysContab.PeriodosDB.Activo(Fecha.DateTime.Date),
                                                                              Fecha.DateTime.Date, "", "", False, 0, "", 2, txtReferencia.Text, Me.Fecha.DateTime.Date, 0,
                                                                              OrigenComprobantes.RENDICION_CUENTA)
            ...
            For i As Integer = 0 To Me.vDatos.DataRowCount - 1
                If IsNull(vDatos.GetRowCellValue(i, "A Pagar"), 0) <> 0 Then
                    ...
                    VB.SysContab.ComprobanteDB.AddComprobanteDetalles(CompNo, vDatos.GetRowCellValue(i, "Cuenta"),
                                                IsNull(vDatos.GetRowCellValue(i, "Proveedor"), "") + IIf(DC = "D", ", " + IsNull(vDatos.GetRowCellValue(i, "#Factura"), ""), ""),
                                                Math.Round(CDbl(Importe), 2),
                                                "0", cbTipo.EditValue, DC, IsNull(vDatos.GetRowCellValue(i, "#Factura"), ""), 0,
            ...
            If MontoMnto > 0.0 Then
                VB.SysContab.ComprobanteDB.AddComprobanteDetalles(CompNo, CuentaMtto,
                                            ...
                                            "0", cbTipo.EditValue, "D", "", 0, Me.Fecha.DateTime.Date, 1)
            End If

            '' Credito
            VB.SysContab.ComprobanteDB.AddComprobanteDetalles(CompNo, cbRendirCuenta.EditValue,
                                            "",
                                            Math.Round(CDbl(vDatos.Columns("A Pagar").SummaryItem.SummaryValue + MontoMnto), 2),
                                            "0", cbTipo.EditValue, "C", "", 0, Me.Fecha.DateTime.Date, 0)
    ...
    'Private Sub cbBanco_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
    '    Try
